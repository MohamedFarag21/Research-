...
class CustomDataset(Dataset):
    ...
    def __init__(self, csv_file, image_folder, target_folder,transform_image=None,transform_target=None):
        ...
        self.image_paths = self.image_paths[0:] #for testing and debugging only we selected the first 100 paths only to test that everything is working propely.
...
mc_prob = monte_carlo_predictions.softmax(dim=2)

mc_avg_runs = monte_carlo_predictions.mean(dim=1)
...
metric = MulticlassCalibrationError(num_classes=43, n_bins=100, norm='l1')
print(metric(mc_avg_runs, all_targets))
